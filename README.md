...
每次抽卡自动计算张力得分，帮助判断正负特质组合的戏剧性：

- **得分范围**：0.0（无张力）~ 1.0（全面对立）
- **计算方式**：实际冲突对数 / 最大可能对数（正面数 × 负面数）
- **推荐区间**：0.25 ~ 0.50（既有冲突又不极端）

Full/Summary 模式下会在结果末尾显示张力指标表：
...
_0.0 = 无张力，1.0 = 全面对立。推荐区间 0.25–0.5_
```

Compact 模式下在标题旁直接显示分数：`**张力**：0.12`

## 数据质量

所有 200 张特质卡的冲突引用经过清洗验证：

- **0 条悬空引用** — 所有 `conflicting_traits` 条目均指向真实存在的特质 key
- **0 条碎片条目** — 无单字符或拆分残留
- **0 条散文/正文混入** — 无标点符号混入（合法 key 如 `古怪的，不可靠的` 除外）
- **0 条英文标注** — 冲突引用中无英文残留
